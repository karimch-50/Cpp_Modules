/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   Fixed.cpp                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: kchaouki <kchaouki@student.1337.ma>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/27 10:55:08 by kchaouki          #+#    #+#             */
/*   Updated: 2023/08/27 11:57:33 by kchaouki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Fixed.hpp"

const int	Fixed::f_bits = 8;

Fixed::Fixed()
{
	number = 0;
	// std::cout << "Default constructor called" << std::endl;
}

Fixed::Fixed(const int _num)
{
	// std::cout << "Int constructor called" << std::endl;
	number = _num << f_bits;
}

Fixed::Fixed(const float _num)
{
	// std::cout << "Float constructor called" << std::endl;
	number = _num * (1 << f_bits);
}

Fixed::~Fixed()
{
	// std::cout << "Destructor called" << std::endl;
}

/*------------------operator overloading-----------------------*/

Fixed& Fixed::operator=(const Fixed& _assignment)
{
	// std::cout << "Copy assignment operator called" << std::endl;
	this->number = _assignment.getRawBits();
	return (*this);
}

Fixed&	Fixed::operator+(const Fixed& _assignment)
{
	this->number += _assignment.number;
	return (*this);
}

Fixed&	Fixed::operator-(const Fixed& _assignment)
{
	this->number -= _assignment.number;
	return (*this);
}

Fixed&	Fixed::operator*(const Fixed& _assignment)
{
	this->number *= _assignment.toFloat();
	return (*this);
}

Fixed&	Fixed::operator/(const Fixed& _assignment)
{
	this->number /= _assignment.number;
	return (*this);
}

bool	Fixed::operator>(const Fixed& _assignment)
{
	return (this->number > _assignment.number);
}

bool	Fixed::operator>=(const Fixed& _assignment)
{
	return (this->number >= _assignment.number);
}

bool	Fixed::operator<(const Fixed& _assignment)
{
	return (this->number < _assignment.number);	
}

bool	Fixed::operator<=(const Fixed& _assignment)
{
	return (this->number <= _assignment.number);	
}

bool	Fixed::operator==(const Fixed& _assignment)
{
	return (this->number == _assignment.number);
}

bool	Fixed::operator!=(const Fixed& _assignment)
{
	return (this->number != _assignment.number);
}

Fixed&	Fixed::operator++()
{
	++number;
	return (*this);
}

Fixed	Fixed::operator++(int)
{
	Fixed fxd = *this;
	++number;
	return (fxd);
}

int& Fixed::min(int& nbr1, int& nbr2)
{
	return (nbr1 < nbr2 ? nbr1 : nbr2);
}

// static int& Fixed::min(const int& nbr1, const int& nbr2);
// static int& Fixed::max(int& nbr1, int &nbr2);
// static int& Fixed::max(const int& nbr1, const int &nbr2);



Fixed::Fixed(const Fixed& _copy)
{
	// std::cout << "Copy constructor called" << std::endl;
	*this = _copy;
}

void		Fixed::setRawBits(int const raw)
{
	// std::cout << "setRawBits member function called" << std::endl;
	number = raw;
}

int	Fixed::getRawBits(void) const
{
	// std::cout << "getRawBits member function called" << std::endl;
	return (number);
}

float		Fixed::toFloat(void) const
{
	return ((float)number / (1 << f_bits));
}

int		Fixed::toInt(void) const
{
	return (number >> f_bits);
}

std::ostream&	operator<<(std::ostream& stream, const Fixed& fixed)
{
	stream << fixed.toFloat();
	return (stream);
}
